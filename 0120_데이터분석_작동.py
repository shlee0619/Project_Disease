...
###############################################
# 4. LSTM용 Dataset 생성 및 Walk-Forward
###############################################
...
def walk_forward_validation_lstm(
    ...
    """
    LSTM용 walk-forward 방법:
    - i 시점에서 (0~i-1)까지로 train_func으로 모델 생성
    - i >= window_size 인 경우, (i-window_size ~ i-1)을 입력으로 예측
    """
    n = len(df)
    df_result = pd.DataFrame(index=df.index, columns=['actual','pred'])
    df_result['actual'] = df[y_col].values
...
def train_lstm_ktuner(
    ...
    # (여기서는 epoch/batch_size를 "하이퍼파라미터"로 넣지 않고,
    #  고정 값으로 설정. 필요하면 build_lstm_model_with_hp 안에서
    #  hp.Choice('epochs', ...) 등을 추가 후 custom Tuner 사용 가능)
    best_epoch = max(epochs_candidates)  # 가장 큰 에폭으로 시도 → 빨리 수렴 시 early stopping
